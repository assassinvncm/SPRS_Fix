...
//	Page<Object[]> getStoreByStatusOrType(@Param("p_user_id") Long p_user_id,@Param("p_status") int p_status,@Param("p_type") Long types, Pageable pageable);
	...
	@Query("select s from Store s JOIN s.store_category sc where s.users.id = :p_user_id and (:p_status = 3 OR s.status = :p_status) AND (CONCAT(:p_type, '') = '0' OR sc.id = :p_type)")//
